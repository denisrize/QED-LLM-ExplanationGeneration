...
def load_aligned_entities(alignment_dict: List[Mapping[Text, Any]],
  ...
  for single_np_alignment in alignment_dict:
    ...
    if q_entity_text != question_text[q_entity_offset[0]:q_entity_offset[1]]:
      message = f"Question entity offset not proper. The reference: {q_entity_text}, the actual text: {question_text[q_entity_offset[0]:q_entity_offset[1]]}"
    ...
    if c_entity_offset[0] != -1:
      if c_entity_text != context_text[c_entity_offset[0]:c_entity_offset[1]]:
        message = f"Context entity offset not proper. The reference: {c_entity_text}, the actual text: {context_text[c_entity_offset[0]:c_entity_offset[1]]}"
        logging.error(message)
        raise ValueError(message)
      doc_entity = Entity(
